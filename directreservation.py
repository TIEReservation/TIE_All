...
def expand_rooms(room_str: str) -> list[str]:
    """Expand room strings: handle ',', '&', 'to' for ranges and combinations."""
    rooms = []
    ...
    for part in parts:
        ...
        if 'to' in part:
            ...
            try:
                rooms.extend([str(i) for i in range(int(start), int(end) + 1)])
            except ValueError:
                rooms.append(part)  # Fallback if not numeric
        elif '&' in part:
            subparts = part.split('&')
            rooms.append(part)  # Add combined
            rooms.extend(subparts)  # Add individuals
        else:
            rooms.append(part)
    return sorted(set(rooms))  # Unique and sorted

def parse_room_data() -> dict:
    """Parse and standardize the provided room data into a nested dict.
    Corrections: 'Deluex' -> 'Deluxe', 'BHA' -> 'BHK', 'Appartment' -> 'Apartment', etc.
    Incorporate notes for breakdowns.
    """
    mapping = {}

    # Le Poshe Beachview
    mapping["Le Poshe Beachview"] = {
        "Double Room": expand_rooms("101,102,202,203,204"),
        "Standard Room": expand_rooms("201"),
        "Deluxe Double Room Seaview": expand_rooms("301,302,303,304")
    }

    # La Millionare Resort
    mapping["La Millionare Resort"] = {
        "Double Room": expand_rooms("101,102,103,105"),
        "Deluxe Double Room with Balcony": expand_rooms("205,304,305"),
        "Deluxe Triple Room with Balcony": expand_rooms("201,202,203,204,301,302,303"),
        "Deluxe Family Room with Balcony": expand_rooms("206,207,208,306,307,308"),
        "Deluxe Triple Room": expand_rooms("402"),
        "Deluxe Family Room": expand_rooms("401")
    }

    # Le Poshe Luxury
    mapping["Le Poshe Luxury"] = {
        "2BHK Apartment": expand_rooms("101&102,101,102"),
        "2BHK Apartment with Balcony": expand_rooms("201&202,201,202,301&302,301,302,401&402,401,402"),
        "3BHK Apartment": expand_rooms("203to205,203,204,205,303to305,303,304,305,403to405,403,404,405"),
        "Double Room with Private Terrace": expand_rooms("501")
    }

    # Le Poshe Suite
    mapping["Le Poshe Suite"] = {
        "2BHK Apartment": expand_rooms("601&602,601,602,603,604,703,704"),
        "2BHK Apartment with Balcony": expand_rooms("701&702,701,702"),
        "Double Room with Terrace": expand_rooms("801")
    }

    # La Paradise Residency
    mapping["La Paradise Residency"] = {
        "Double Room": expand_rooms("101,102,103,301,304"),
        "Family Room": expand_rooms("201,203"),
        "Triple Room": expand_rooms("202,303")
    }

    # La Paradise Luxury (with note breakdown)
    mapping["La Paradise Luxury"] = {
        "3BHK Apartment": expand_rooms("101to103,201to203"),
        "Double Room": expand_rooms("101,201"),  # From note
        "Master Room": expand_rooms("102,103,202,203")  # From note
    }

    # La Villa Heritage (with note breakdown)
    mapping["La Villa Heritage"] = {
        "Double Room": expand_rooms("101,102,103"),
        "4BHK Apartment": expand_rooms("201to203&301,201,202,203,301"),
        "Master Room": expand_rooms("202,203")  # From note
    }

    # Le Pondy Beach Side
    mapping["Le Pondy Beach Side"] = {
        "Villa": expand_rooms("101to104,101,102,103,104")
    }

    # Le Royce Villa
    mapping["Le Royce Villa"] = {
        "Villa": expand_rooms("101to102&201to202,101,102,201,202")
    }

    # La Tamara Luxury
    mapping["La Tamara Luxury"] = {
        "3BHK": expand_rooms("101to103,101,102,103,104to106,104,105,106,201to203,201,202,203,204to206,204,205,206,301to303,301,302,303,304to306,304,305,306"),
        "4BHK": expand_rooms("401to404,401,402,403,404")
    }

    # La Antilia
    mapping["La Antilia"] = {
        "Deluxe Suite Room": expand_rooms("101"),
        "Deluxe Double Room": expand_rooms("203,204,303,304"),
        "Family Room": expand_rooms("201,202,301,302"),
        "Deluxe Suite Room with Terrace": expand_rooms("404")
    }

    # La Tamara Suite
    mapping["La Tamara Suite"] = {
        "Two Bedroom Apartment": expand_rooms("101&102"),
        "Deluxe Apartment": expand_rooms("103&104"),
        "Deluxe Double Room": expand_rooms("203,204,205"),
        "Deluxe Triple Room": expand_rooms("201,202"),
        "Deluxe Family Room": expand_rooms("206")
    }

    # Le Park Resort
    mapping["Le Park Resort"] = {
        "Villa with Swimming Pool View": expand_rooms("555&666,555,666"),
        "Villa with Garden View": expand_rooms("111&222,111,222"),
        "Family Retreat Villa": expand_rooms("333&444,333,444")
    }

    # Villa Shakti
    mapping["Villa Shakti"] = {
        "2BHK Studio Room": expand_rooms("101&102"),
        "2BHK with Balcony": expand_rooms("202&203,302&303"),
        "Family Suite": expand_rooms("201"),
        "Family Room": expand_rooms("301"),
        "Terrace Room": expand_rooms("401")
    }

    # Eden Beach Resort
    mapping["Eden Beach Resort"] = {
        "Double Room": expand_rooms("101,102"),
        "Deluxe Room": expand_rooms("103,202"),
        "Triple Room": expand_rooms("201")
    }

    return mapping

property_room_mapping = parse_room_data()
...
def show_new_reservation_form():
    ...
    with col1:
        ...
        # Dynamic room types
        available_types = sorted(property_room_mapping.get(property_name, {}).keys()) + ["Other"]
        room_type = st.selectbox("Room Type", available_types, key=f"{form_key}_roomtype", help="Available types for this property")
        
        if room_type == "Other":
            custom_room_type = st.text_input("Custom Room Type", key=f"{form_key}_custom_roomtype")
            room_no = st.text_input("Room No", placeholder="e.g., 101, 202", key=f"{form_key}_room", help="Enter custom room number")
        else:
            custom_room_type = None
            available_rooms = property_room_mapping[property_name][room_type]
            room_no = st.selectbox("Room No", available_rooms, key=f"{form_key}_room", help="Available rooms for this type")
...
def show_edit_form(edit_index):
    ...
    with col1:
        ...
        # Dynamic room types
        available_types = sorted(property_room_mapping.get(property_name, {}).keys()) + ["Other"]
        existing_type = reservation["Room Type"]
        type_index = available_types.index(existing_type) if existing_type in available_types else available_types.index("Other")
        room_type = st.selectbox("Room Type", available_types, index=type_index, key=f"{form_key}_roomtype", help="Available types for this property")
        
        if room_type == "Other":
            custom_room_type = st.text_input("Custom Room Type", value=existing_type if room_type == "Other" else "", key=f"{form_key}_custom_roomtype")
            room_no = st.text_input("Room No", value=reservation["Room No"], key=f"{form_key}_room", help="Enter custom room number")
        else:
            custom_room_type = None
            available_rooms = property_room_mapping[property_name][room_type]
            room_index = available_rooms.index(reservation["Room No"]) if reservation["Room No"] in available_rooms else 0
            room_no = st.selectbox("Room No", available_rooms, index=room_index, key=f"{form_key}_room", help="Available rooms for this type")
...
if __name__ == "__main__":
    ...
    menu = ["New Reservation", "View Reservations", "Edit Reservations", "Analytics"]
    choice = st.sidebar.selectbox("Menu", menu)

    if choice == "New Reservation":
        show_new_reservation_form()
    elif choice == "View Reservations":
        show_reservations()
    elif choice == "Edit Reservations":
